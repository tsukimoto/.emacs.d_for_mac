...
;; Copyright 2011-2013 Fran∫ois-Xavier Bois

;; Version: 7.0.25
;; Author: Fran∫ois-Xavier Bois <fxbois AT Google Mail Service>
...
;;todo : auto-pairs deviennent sp・cifiques ≒ un engine
;;todo : reduction des css rules div { ... }
;;todo : passer les content-types en symboles
;;todo : tester shortcut A -> pour pomme
;;todo : commentaire d'une ligne ruby ou d'une ligne asp
;;todo : ne mettre tag-type et tag-name que sur le '<'
;;todo : cr・er tag-token pour diff・rentier de part-token : tag-token=attr,comment ???
...
;; todo : il serait pr・f・rable d'identifier les tokens et ensuite de fontifier
  ...
  (let (continue)
    (cond
     ...
     (t
      ...
      (setq continue t)
      (while continue
        (skip-chars-forward " a-zA-Z0-9_-"); caract∬re 'espace' pour '} else {'
        (cond
         ((eq ?\( (char-after))
          (search-forward ")")
          )
...
;; doit-on consid・rer que '=' est un bloc ouvrant avec ';' comme char de fin ?
...
(defun web-mode-previous-line (pos limit)
  ...
  (save-excursion
    (let (beg end line (continue t))
      ...
      (if (<= (point) limit)
          ;;todo : affiner (le + 3 n est pas g・n・rique cf. <?php <% <%- etc.)
  ...
  (let ((beg pos) (end pos) prop reg-beg boundaries)
    ...
    ((and (member (get-text-property pos 'block-token) '(comment string))
     ...
     ((and (eq (get-text-property pos 'part-token) 'attr)
      ...
      ;; todo: tester que le car pr・c・dent n'est pas un
      (when (eq (get-text-property pos 'part-token) (get-text-property (1- pos) 'part-token))
        (setq beg (previous-single-property-change pos 'part-token)))
      (when (eq (get-text-property pos 'part-token) (get-text-property (1+ pos) 'part-token))
        (setq end (next-single-property-change pos 'part-token)))
...
(defun web-mode-entities-replace ()
  "Replace HTML entities ie. &eacute; becomes ・"
  (interactive)
  (save-excursion
...
(defun web-mode-trace (msg)
  ...
  (when nil
    ...
    (message "%18s: time elapsed = %Ss %9S・s" msg (nth 1 sub) (nth 2 sub))
    ))
